...
def batch_to_feed_dict(batch, is_train):
    ...
    relevance_mask_np = [(np.arange(len(seq_len_np)) < seq_len_np[i]) / seq_len_np[i] for i in range(n_batch)]
    ...
    return {features: features_np, phns: phns_labels, seq_len: seq_len_np, weighted_relevant_mask: relevance_mask_np,
            batch_size: n_batch, keep_prob: FLAGS["drop_out_probability"] if is_train else 1.}
...
if __name__ == '__main__':
        ...
        def bi_directional_lstm(inputs, layer_number):
            with tf.variable_scope('BiDirectionalLayer' + str(layer_number)):
                if layer_number == 0:
                    ...
                    cell_f = get_cell("FW" + str(layer_number), n_input=(FLAGS["n_regular"] + FLAGS["n_adaptive"]))
    ...
    with tf.name_scope('Output'):
        ...
        w_out = tf.Variable(rd.randn(n_outputs, N_output_classes) / np.sqrt(n_outputs),
                            dtype=tf.float32, name="OutWeights")

        b_out = tf.Variable(np.zeros(N_output_classes), dtype=tf.float32, name="OutBias")
    ...
    while dataset.current_epoch <= FLAGS["n_epochs"]:
        ...
        if (dataset.current_epoch != current_epoch) or (k_iteration == 0):
            ...
            print('Epoch: {} \t loss {:.3g} (train) \t Acc: {:.3g} (train) \t'
                'ACC: {:.3g} (test)'
                .format(dataset.current_epoch, train_loss, train_acc, test_result['acc']))
        ...
        _, train_result = sess.run([train_step, train_result_tensors], feed_dict=batch_to_feed_dict(dataset.get_next_training_batch(), is_train=True))
        train_results.append(train_result)

    complete_time = time.time() - t0

    save_experiment_results(FLAGS, complete_time, overall_results, f'solve_hdd_lsnn_{"BPTT" if EPROP is None else "eprop"}_{datetime.now().strftime("%Y_%m_%d_%H_%M_%S")}')
