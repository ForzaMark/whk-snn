...
def batch_to_feed_dict(batch):
    ...
    return {features: features_np, phns: phns_np, weighted_relevant_mask: relevance_mask_np}
...
tf.app.flags.DEFINE_string('preproc', 'mfccs', 'Input preprocessing: fbank, mfccs, cochspec, cochspike, htk')
...
with tf.name_scope('Output'):
    w_out_init = rd.randn(FLAGS.n_lstm * 2, dataset.n_classes) / np.sqrt(FLAGS.n_lstm * 2)  # original
...
while dataset.current_epoch <= FLAGS.n_epochs:
    ...
    if (dataset.current_epoch) != current_epoch or (k_iteration == 0):
        ...
        test_result = sess.run(test_result_tensors, feed_dict=batch_to_feed_dict(dataset.get_test_batch()))
        ...
        print('Epoch: {} \t loss {:.3g} (train) \t acc: {:.3g} (train) \t acc: {:.3g} (test)'.format(dataset.current_epoch, train_loss, train_acc, test_result['acc']))
        
    _, train_res = sess.run([train_step, train_result_tensors], feed_dict=batch_to_feed_dict(dataset.get_next_training_batch()))
...
}, complete_time, overall_results, f'solve_hdd_lstm_{"BPTT" if FLAGS.eprop is None else "eprop"}_{datetime.now().strftime("%Y_%m_%d_%H_%M_%S")}')
