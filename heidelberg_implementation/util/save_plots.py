...
def save_loss_per_time_step_plot(epoch_loss_per_time_step, path):
    x = range(100)
    ys_last = [
        (value["epoch"], value["loss_per_time_step_last_element"])
        for value in epoch_loss_per_time_step
    ]
    ys_first = [
        (value["epoch"], value["loss_per_time_step_first_element"])
        for value in epoch_loss_per_time_step
    ]
    ys_averaged = [
        (value["epoch"], value["loss_per_time_step_averaged_element"])
        for value in epoch_loss_per_time_step
    ]

    ys = [ys_last, ys_first, ys_averaged]
